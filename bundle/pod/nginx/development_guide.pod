...
The request flag C<reading_body> indicates that the request body
reading is still in progress.
To read the next part, call the
C<ngx_http_read_unbuffered_request_body(r)> function.
The return value C<NGX_AGAIN> indicates that more body data
or buffer space is needed.
If C<bufs> is NULL after calling this function, no body data is
...
The
L<nginx-dev-examples|https://github.com/nginx/nginx-dev-examples>
...
L<ngx_http_auth_request module|https://github.com/nginx/nginx/blob/master/src/http/modules/ngx_http_auth_request_module.c>.
...
available in nginx.
For example,
L<OCSP module|https://github.com/nginx/nginx/blob/master/src/event/ngx_event_openssl_stapling.c>
