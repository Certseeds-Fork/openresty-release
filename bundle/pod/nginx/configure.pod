...
This module is not built by default.
The OpenSSL library that provides HTTPE<sol>3 support
...
=item 
C<--without-http_upstream_least_time_module>
...
=item 
C<--without-http_upstream_sticky_module>
...
disables building a module that provides
L<session
affinity|ngx_http_upstream_module> to upstream servers.
...
=item 
C<--without-stream_upstream_least_time_module>
