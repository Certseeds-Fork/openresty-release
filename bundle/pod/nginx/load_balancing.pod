...
=head1 Least time load balancing
...
L<least_time|ngx_http_upstream_module> directive:
...
C<header> — time to receive the first byte from the server,



=item *

C<last_byte> — time to receive the full response from the server,



=item *

C<last_byte inflight> — time to receive the full response from
...
L<least_time|ngx_http_upstream_module> directive is used as part of the server group configuration:
