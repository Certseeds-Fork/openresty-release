...
=head2 proxy_allow_upstream


B<syntax:> proxy_allow_upstream I<I<C<string>> ...>
...
=head2 proxy_bind_dynamic


B<syntax:> proxy_bind_dynamic I<C<on> E<verbar> C<off>>
...
B<syntax:> proxy_http_version I<
    C<1.0> E<verbar> C<1.1> E<verbar>
    C<2>>
...
L<explicitly enabled|https://datatracker.ietf.org/doc/html/rfc9110#section-6.5.1>:
...
HTTPE<sol>1.1 or HTTPE<sol>2 is enabled for proxying.
...
=head2 proxy_request_dynamic


B<syntax:> proxy_request_dynamic I<C<on> E<verbar> C<off>>
...
For example, the server-specific C<Host> request header field
can be set:
...
If HTTPE<sol>1.0 or HTTPE<sol>1.1 is
...
unless it is replaced with an explicit C<Host> header field.
