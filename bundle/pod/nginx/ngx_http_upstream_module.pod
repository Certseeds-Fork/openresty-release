...
B<syntax:> keepalive I<I<C<connections>> [C<local>]>


B<default:> I<32 local>
...
L<locations|ngx_http_core_module>.





The C<local> parameter (1.29.7) disables sharing of
cached keepalive connections across different locations
even if the upstream server address matches.
If the C<local> parameter is not specified,
...
default limit of I<C<32>> connections per each worker process.
...
directive should be “C<1.1>” (by default since 1.29.7)
or set to “C<2>”
